...
sy = math.sqrt(rotation_matrix_unity[0,0] * rotation_matrix_unity[0,0] +  rotation_matrix_unity[0,1] * rotation_matrix_unity[0,1])
singular = sy < 1e-6

if not singular:
    y = math.atan2(rotation_matrix_unity[2,0], rotation_matrix_unity[0,0])
    x = math.atan2(-rotation_matrix_unity[1,2], rotation_matrix_unity[1,1])
    z = math.atan2(rotation_matrix_unity[1,0], rotation_matrix_unity[1,1])
else:
    y = math.atan2(-rotation_matrix_unity[0,2], rotation_matrix_unity[2,2])
    x = math.atan2(-rotation_matrix_unity[1,0], sy)
    z = 0

# Convertir a grados
euler_angles_unity = np.degrees([y, x, z])
...
if abs(euler_angles_unity[0] + 180) < 1e-5:
    euler_angles_unity[0] = -180
